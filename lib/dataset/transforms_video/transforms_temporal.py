    ...
    def __call__(self, frame_indices: np.ndarray):
        ...
        if selected is not None:
            ...
            if self.n == 1:
                ...
                start_index = np.linspace(0, num_frames - needed_frames,
                                          num=self.n).round().astype(int)
...
class Resample:
    def __init__(self, logger, target_fps: float = 30.):
...
class LimitRange:
    def __init__(self, logger, min_frames: int, limit_rate=1):
