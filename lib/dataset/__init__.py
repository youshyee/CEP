...
from .transforms_video import (transforms_spatial, transforms_temporal, transforms_tensor)
...
class DataLoaderFactory:
    ...
    def build(self, split='train', device=None, pretrain=True):
        ...
        self.logger.info(f'Building Dataset:{stat} on {ds_name}, Split={split}')
        ...
        gpu_collate_fn = transforms_tensor.SequentialGPUCollateFn(
            ...
            target_transform=(not pretrain),  # pretrain task does not need labels
            device=device,
        )
    ...
    def get_transform(self, split='train'):
        ...
        if split == 'train':
            ...
            cpu_transform = transforms_tensor.Compose([transforms_spatial.RawVideoCenterMaxCrop()])
