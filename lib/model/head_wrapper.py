...
class HeadWrapper(nn.Module):
    ...
    def __init__(
        ...
        if self.finetune:
            ...
            if fc_type == 'linear':
                self.fc1 = self._get_linear_fc(feat_dim, self.op_dim, bn=projectorbn[0])
                self.fc2 = self._get_linear_fc(feat_dim, self.op_dim, bn=projectorbn[1])
            elif fc_type == 'mlp':
                self.fc1 = self._get_mlp_fc(feat_dim, self.op_dim, bn=projectorbn[0])
                self.fc2 = self._get_mlp_fc(feat_dim, self.op_dim, bn=projectorbn[1])
            elif fc_type == 'conv':
                if projectorbn[0]:
                    self.fc1 = ConvBnFc(feat_dim, self.op_dim, (3, 3, 3), (1, 1, 1))
                else:
                    self.fc1 = ConvFc(feat_dim, self.op_dim, (3, 3, 3), (1, 1, 1))
                if projectorbn[1]:
                    self.fc2 = ConvBnFc(feat_dim, self.op_dim, (3, 3, 3), (1, 1, 1))
                else:
                    self.fc2 = ConvFc(feat_dim, self.op_dim, (3, 3, 3), (1, 1, 1))
            elif fc_type == 'speednet':
                self.fc1 = self._get_linear_fc(feat_dim, self.op_dim, bn=projectorbn[0])
                self.fc2 = self._get_linear_fc(feat_dim, 1, bn=projectorbn[1])
    ...
    def _get_feat_dim(self, encoder):
        ...
        for fc_name in fc_names:
            if hasattr(encoder, fc_name):
                ...
                self.logger.info(f'Found fc: {fc_name} with in_features: {feat_dim}')
                break
        return feat_dim

    @staticmethod
    def _get_linear_fc(feat_dim: int, opdim: int, bn=False):
        if bn:
            return nn.Sequential(
                nn.AdaptiveAvgPool3d((1, 1, 1)),
                Flatten(),
                nn.BatchNorm1d(feat_dim),
                nn.Linear(feat_dim, opdim),
            )
        else:
            return nn.Sequential(
                nn.AdaptiveAvgPool3d((1, 1, 1)),
    ...
    @staticmethod
    def _get_mlp_fc(feat_dim: int, opdim: int, bn=False):
        if bn:
            return nn.Sequential(nn.AdaptiveAvgPool3d((1, 1, 1)), Flatten(),
                                 nn.Linear(feat_dim, feat_dim), nn.BatchNorm1d(feat_dim),
                                 nn.ReLU(inplace=True), nn.Linear(feat_dim, opdim))
        else:
            return nn.Sequential(nn.AdaptiveAvgPool3d((1, 1, 1)), Flatten(),
